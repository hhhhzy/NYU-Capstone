...
class Tranformer(nn.Module):
    def __init__(self,feature_size=250,num_enc_layers=1,num_dec_layers=1,d_ff = 256, dropout=0.1,num_head=2, pe_type='3d',grid_size=16):
        ...
        self.ff = nn.Linear(feature_size, feature_size)
        self.ff2 = nn.Linear(feature_size, feature_size//2)
        self.ff22 = nn.Linear(feature_size//2, 1)
        self.ff3 = nn.Linear(feature_size//2, feature_size//4)
        self.ff4 = nn.Linear(feature_size//4, 1)
        self.relu = nn.ReLU()
    ...
    def forward(self, src, tgt, src_coord, tgt_coord, src_ts, tgt_ts):
        ...
        ## add input noise
        src += torch.empty(list(src.size())).normal_(0., 1.).to('cuda:0')
        if self.pe_type == '1d':
        ...
        ### add noise
        #output_enc += torch.empty(list(output_enc.size())).normal_(0.,1.).to('cuda:0')
        ...
        output_dec = self.ff(output_dec)
        output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        output_dec = self.relu(output_dec)
        #output_dec = self.relu(output_dec)    
        # output_dec = self.ff(output_dec)
        # output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        ...
        # output_dec = self.relu(self.ff2(output_dec))
        # output_dec = self.relu(self.ff3(output_dec))
        # output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        # output = self.ff4(output_dec)
        
        # output = self.relu(output_dec)
        ...
        # output_dec = self.ff(output_dec)
        # #output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        # output_dec = self.lrelu(output_dec)

        # output_dec = self.ff2(output_dec)
        # #output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        # output_dec = self.lrelu(output_dec)

        # output_dec = self.ff22(output_dec)
        # output_dec += torch.empty(list(output_dec.size())).normal_(0., 1.).to('cuda:0')
        # output = self.lrelu(output_dec)
